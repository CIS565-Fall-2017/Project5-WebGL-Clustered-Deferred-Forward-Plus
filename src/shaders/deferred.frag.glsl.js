export default function(params) {
  ...
  void main() {
    ...
    if(-view.z >= near) {
      float n = log(-view.z - near + 1.0) / log(u_screenbuffer.w - near + 1.0);
    ...
    for (int lightIndex = 1; lightIndex <= num_lights; lightIndex++) {
      ...
      Light lite = UnpackLight(index);
      float lightDistance = distance(lite.position, worldSpacePos.xyz);
      float lightIntensity = cubicGaussian(2.0 * lightDistance / lite.radius); 
      vec4 viewSpacePos = u_viewMatrix * (vec4(worldSpacePos.xyz, 1.0));
      vec3 L = (lite.position - worldSpacePos.xyz) / lightDistance;
      L = vec3(u_viewMatrix * vec4(L, 0.0));
      vec3 V = -normalize(vec3(viewSpacePos));
      vec3 H = normalize(L + V);
      
      float specular = pow(max(dot(H, normal.xyz), 0.0), 100.0);

      fragColor += (albedo.xyz + vec3(specular)) * max(dot(L, normal.xyz), 0.0) * lightIntensity * lite.color;
    }
    
    const vec3 ambientLight = vec3(0.025);
    //fragColor += albedo.xyz;
    
    fragColor += normal.xyz * ambientLight;
