export default function(params) {
  ...
  void main() {
    ...
    for (int lightIndex = 1; lightIndex <= num_lights; lightIndex++) {
      ...
      float lightDistance = distance(lite.position, v_position);
      float lightIntensity = cubicGaussian(2.0 * lightDistance / lite.radius);
      vec4 cameraWorldPos = u_invViewMatrix * vec4(0.0, 0.0, 0.0, 1.0);
      vec3 L = (lite.position - v_position) / lightDistance;  
      vec3 V = normalize(cameraWorldPos.xyz - v_position);
      vec3 H = normalize(L + V);

      float specular = pow(max(dot(H, normal), 0.0), 100.0);

      fragColor += (albedo + vec3(specular)) * max(dot(L, normal), 0.0) * lightIntensity * lite.color;
    }
