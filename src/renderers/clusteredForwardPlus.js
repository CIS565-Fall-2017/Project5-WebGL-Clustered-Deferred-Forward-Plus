...
import fsSourceRTT from '../shaders/clusteredForwardRTT.frag.glsl.js';
...
export default class ClusteredForwardPlusRenderer extends ClusteredRenderer {
  ...
  renderBloom(camera, scene){
    ...
    gl.uniform1i(this._shaderProgramRTT.u_clusterbuffer, 3);

    gl.uniform2f(this._shaderProgramRTT.u_cluster_tile_size, (canvas.width )/this._xSlices, (canvas.height)/this._ySlices);
    gl.uniform1f(this._shaderProgramRTT.u_nearClip, camera.near);
    gl.uniform1f(this._shaderProgramRTT.u_cluster_depth_stride, (camera.far - camera.near) / this._zSlices);
    gl.uniformMatrix4fv(this._shaderProgramRTT.u_viewMatrix, false, this._viewMatrix);
