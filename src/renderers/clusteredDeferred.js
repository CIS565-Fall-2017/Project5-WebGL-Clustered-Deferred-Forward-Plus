...
export default class ClusteredDeferredRenderer extends ClusteredRenderer {
  constructor(xSlices, ySlices, zSlices) {
    ...
    this._progShade = loadShaderProgram(QuadVertSource, fsSource({
      ...
      uniforms: ['u_gbuffers[0]', 'u_gbuffers[1]', 'u_gbuffers[2]', 'u_gbuffers[3]',
                  'u_viewProjectionMatrix', 'u_viewMatrix', 'u_inverseViewMatrix', 
                  'u_screenHeight','u_screenWidth', 'u_camNear', 'u_camFar',
  ...
  render(camera, scene) {
    ...
    gl.uniform1f(this._progShade.u_camFar, camera.far);
    gl.uniform1f(this._progShade.u_screenHeight, canvas.height);
    gl.uniform1f(this._progShade.u_screenWidth, canvas.width);


    // Bind g-buffers
    // Since I have light and cluster textures at 2 and 3, start this at 4
    const firstGBufferBinding = 4;//0; // You may have to change this if you use other texture slots
